...
/**
 * Redondea un valor a entero (para precios finales sin decimales, según lógica de negocio).
 */
...
// ---------------------------------------------------
// 🚀 CORRECCIÓN: FUNCIONES DE FORMATO DE MONEDA
// ---------------------------------------------------

/**
 * Formatea un número como moneda CLP, manteniendo decimales (para costos y métricas históricas).
 */
function formatCurrencyWithDecimals(amount) {
    // Usar el currencyFormatter global sin redondear a entero
    return currencyFormatter.format(amount);
}

/**
 * Formatea un número como moneda CLP, forzando a entero (para precios finales y totales).
 */
function formatCurrency(amount) {
    // Mantener esta función para precios finales (redondeados por lógica de negocio)
...
function updateInvoiceTotals() {
  ...
  INVOICE_ITEMS.forEach(item => {
    const netCost = parseFloat(item.cost_price_net_new || 0); // Costo Neto (flotante)
    // const grossCostPerUnit = calculateGrossCost(netCost); // ❌ ¡ELIMINADO! Ya no recalculamos.
        
        // ✅ USAMOS EL VALOR BRUTO ALMACENADO (de 4 decimales)
        const grossCostPerUnit = parseFloat(item.cost_price_gross_new || 0);
        
    const quantity = parseInt(item.quantity || 0);

    // Sumar como flotantes
    subtotalNet += netCost * quantity;
    subtotalGross += grossCostPerUnit * quantity; // Ahora usa el valor almacenado (item.cost_price_gross_new)
  });
  ...
  // Usamos las versiones redondeadas para mostrar (usando formatCurrency que aplica toIntegerPrice)
...
function onCostInputChange(input, index) {
    ...
    if (input === inputGrossCost) {
        ...
        // 🚀 CLAVE: Usamos toFixed(2) para mostrar 2 decimales y forzar la entrada del usuario a 2 decimales.
    ...
    onSalePriceUpdate(row, index, currentMarginPct, 'margin'); // Pasamos 'margin' para forzar el cálculo
...
function onSalePriceInputChange(input, index) {
    ...
    if (input === inputFinalPrice) {
        ...
        // Aseguramos que el valor ingresado tenga 2 decimales para la vista
        ...
        // El precio final de venta aún pasa por la función de redondeo del negocio (que devuelve entero).
        ...
        // 🚀 CLAVE: Usamos toFixed(2) para que el valor "X.00" se muestre, pero el valor es entero.
        inputFinalPrice.value = finalPrice.toFixed(2); 
...
function onSalePriceUpdate(row, index, value, type) {
    ...
    if (type === 'none') {
        ...
        const suggestedNetPrice = calculateSalePriceFromMargin(netCost, marginPct);
        const suggestedGrossPrice = calculateGrossCost(suggestedNetPrice);
        // Aquí finalPrice se convierte en el valor ENTERO redondeado por la lógica de negocio
        finalPrice = roundToNearestHundredMinusTen(suggestedGrossPrice);
    } else { // type === 'price'
        // Aquí finalPrice mantiene los decimales ingresados por el usuario (ej: 5.6)
        finalPrice = value; 
        marginPct = calculateMarginPercentage(netCost, finalPrice);
    }
    
    // Recalcular el valor sugerido (siempre basado en el MARGEN ACTUAL)
    const currentMarginInput = parseFloat(row.querySelector('.input-margen-pct').value) || 0;
    const suggestedNetPrice = calculateSalePriceFromMargin(netCost, currentMarginInput);
    const suggestedGrossPriceUnrounded = calculateGrossCost(suggestedNetPrice);
    const suggestedFinalRoundedPrice = roundToNearestHundredMinusTen(suggestedGrossPriceUnrounded); // Precio sugerido redondeado final (ENTERO)

    // Actualizar Interfaz (Display Sugerido)
    row.querySelector('.display-precio-sug').textContent = formatCurrency(suggestedFinalRoundedPrice); // Usa formatCurrency (con toIntegerPrice)

    // Actualizar Inputs
    if (type !== 'price') {
        // Si el origen NO fue el precio final, se sobreescribe con el valor calculado
        // 🚀 CLAVE: Usamos toFixed(2) para mostrar, pero ya sabemos que si viene de margen, finalPrice es entero (ej: 5.00).
        row.querySelector('.input-precio-final').value = finalPrice.toFixed(2); 
    }
    if (type !== 'margin') {
    ...
    // 🚀 CLAVE: Usamos el valor del input para asegurar que si el usuario ingresó decimales (ej: 5.6), se mantengan.
    // Si viene de cálculo de margen, el input ya se actualizó a X.00 (entero).
    item.new_sale_price = parseFloat(row.querySelector('.input-precio-final').value); 
    item.quantity = parseInt(row.querySelector('.input-quantity').value) || 0;
...
function addRowListeners(row, index) {
    ...
    inputMargin.addEventListener('input', () => {
        ...
        // 🚀 CLAVE: Actualiza el input de precio final y el display sugerido en tiempo real
        inputFinalPrice.value = finalPrice.toFixed(2); // Usamos toFixed(2) para la vista X.00
        const display = row.querySelector('.display-precio-sug');
        if (display) display.textContent = formatCurrency(finalPrice); // Usa formatCurrency (con toIntegerPrice)
    });
    ...
    onCostInputChange(inputNetCost, index); // Esto fuerza un primer cálculo de venta.
}
...
function renderInvoiceItem(product, shouldRenderCards = true) {
  ...
  const productId = product.id;
  // Usar parseFloat() para los costos de producto (sin redondear aquí)
  ...
  const newItem = {
    ...
    // 🚀 CLAVE: new_sale_price se inicializa con el precio actual (que puede ser flotante)
    new_sale_price: parseFloat(product.current_sale_price || 0),
    new_margin: product.current_margin || 0, // Se inicializa con el margen/precio de venta actual
    is_new: product.is_new || false,
    // Almacenamos el costo/precio actual para referencia (flotantes)
    current_cost_price_net: parseFloat(product.cost_price_net),
    current_cost_price_gross: calculateGrossCost(product.cost_price_net),
    current_sale_price: parseFloat(product.current_sale_price),
...
row.innerHTML = `
  ...
  <td class="text-right">${formatCurrencyWithDecimals(newItem.current_cost_price_net)}</td>
  <td class="text-right">${formatCurrencyWithDecimals(newItem.current_cost_price_gross)}</td>
  <td class="text-right">${newItem.current_margin.toFixed(2)}%</td>
  <td class="text-right">${formatCurrencyWithDecimals(newItem.current_sale_price)}</td>

  <td>
   <input type="number" class="invoice-input input-costo-neto"
        value="${initialNetCost.toFixed(COST_PRECISION)}" step="0.01" min="0" required>
  </td>
  <td>
   <input type="number" class="invoice-input input-costo-bruto-nuevo"
        value="${initialGrossCost.toFixed(COST_PRECISION)}" step="0.01" min="0" required>
  </td>
  <td>
  ...
  <td>
   <input type="number" class="invoice-input input-precio-final" value="${newItem.new_sale_price.toFixed(2)}" step="0.01" min="0" required>
  </td>
  <td>
   <input type="number" class="invoice-input input-quantity" value="${newItem.quantity}" step="1" min="1" required>
  </td>
  <td>
   <button class="btn-remove btn-danger" data-id="${productId}">Borrar</button>
  </td>
 `;
...
function removeInvoiceItem(productId) {
    ...
    // Volvemos a renderizar la grilla.
    renderProductCards(ALL_SUPPLIER_PRODUCTS); 
...
function clearInvoice(suppressMessage = false) {
    ...
    // Re-renderizar la grilla para que todos los productos se habiliten
    renderProductCards(ALL_SUPPLIER_PRODUCTS); 
...
function reRenderInvoice() {
  ...
  INVOICE_ITEMS.forEach((item, index) => {
    ...
    row.innerHTML = `
      ...
      <td class="text-right">${formatCurrencyWithDecimals(currentNetCost)}</td>
      <td class="text-right">${formatCurrencyWithDecimals(currentGrossCost)}</td>
      <td class="text-right">${currentMargin.toFixed(2)}%</td>
      <td class="text-right">${formatCurrencyWithDecimals(currentSalePrice)}</td>

      <td>
        <input type="number" class="invoice-input input-costo-neto" 
                     value="${parseFloat(item.cost_price_net_new).toFixed(COST_PRECISION)}" step="0.01" min="0" required>
      </td>
      <td>
        <input type="number" class="invoice-input input-costo-bruto-nuevo" 
                     value="${parseFloat(item.cost_price_gross_new).toFixed(COST_PRECISION)}" step="0.01" min="0" required>
      </td>
      <td>
        <input type="number" class="invoice-input input-margen-pct" value="${initialMargin}" step="0.01" min="0" required>
      </td>
      ...
      <td>
        <input type="number" class="invoice-input input-precio-final" value="${parseFloat(item.new_sale_price).toFixed(2)}" step="0.01" min="0" required>
      </td>
      <td>
        <input type="number" class="invoice-input input-quantity" value="${item.quantity}" step="1" min="1" required>
      </td>
      <td>
        <button class="btn-remove btn-danger" data-id="${item.id}">Borrar</button>
      </td>
    `;
  ...
  // Renderizar la grilla al recargar la tabla para actualizar el estado visual
  renderProductCards(ALL_SUPPLIER_PRODUCTS);
}
// =========================================================
...
function renderProductCards(products) {
    ...
    const cardsHtml = finalProducts.map(product => {
        ...
        // 🚀 CLAVE: Usamos formatCurrencyWithDecimals para el costo neto
        const currentCostNet = formatCurrencyWithDecimals(parseFloat(product.cost_price_net)); 
    ...
    // Adjuntar listeners de clic sin cerrar el modal y con scroll
            ...
            if (product) {
                ...
                if (success) {
                    ...
                    if(lastRow) {
                        ...
                        // 2. Forzar el scroll a la fila en la tabla principal.
...
function handleNewProductSubmission(e) {
    ...
    // Usar parseFloat para leer el valor del input, aunque luego se redondee a entero
    ...
    const newProduct = {
        ...
        name: name,
        stock: 0,
        // Almacenar los costos como flotantes
        ...
        current_sale_price: roundToNearestHundredMinusTen(priceFinal), 
        quantity: quantity,
        is_new: true,
        image_url: null, 
    ...
    // Usamos la función de adición centralizada
...
function handleFinalInvoiceSubmission(e) {
    ...
    // Calcular el total con flotantes antes de redondear a entero para el envío final
    ...
    const payload = {
        ...
        items: INVOICE_ITEMS.map(item => ({
            ...
            // 🚀 CLAVE: Enviar los costos con precisión decimal al backend
            cost_price_net: parseFloat(item.cost_price_net_new).toFixed(4), // Usar 4 decimales para mayor precisión en PHP
            // 🚀 CLAVE: Enviar el precio de venta (flotante) tal como está en el item (decimales incluidos si se ingresaron)
            new_sale_price: parseFloat(item.new_sale_price).toFixed(4), 
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            // 3. Recargar solo el historial después de un breve retraso
            setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    registerInvoiceBtn.addEventListener('click', (e) => {
        // Detener la propagación para evitar que el modal se cierre al instante.
        e.preventDefault(); 
        e.stopPropagation(); 
